"""Insurgency 2014 `.nav` parser — the shipped navigation mesh, as the game sees it.

Areas with their corner positions, inter-area connections, hiding spots, light
intensity and visibility data, from Source `.nav` version 16 subversion 4.

This is the authority on where a player can *stand*: `bspscout.nav` builds a walkable
grid out of BSP geometry, but a pure BSP-tree descent cannot see `func_detail`, which
is most of a map's interior walls. Area footprints here are trustworthy.

Area **connections** are not — see docs/spawns-and-objectives.md §4. On ministry_coop
the parser stops at area 3102 of 3128 and leaves the graph in disjoint components, so
it cannot be used for reachability.

Vendored from the sibling nav-mesh-parser project so this repo stands alone.
"""
